...
整段 FP–BR 的官方肩部總轉移則由平均功率主導，區間時間仍提供明顯的第二層訊號。以100位投手為單位，體重單獨解釋46.9%；體重加 full-signal 重建平均總功率的CV R²為0.652，再加入FP–BR時間升至0.899。這個整段結果不能與下文「軀幹限制期STP」混為同一分析範圍。
...
以完整 `fp_poi_time` 至 BR 為同一積分窗，分別積分 full-signal STP、JFP及兩者總和。411球與100位投手均完整保留；投手平均FP–BR時間0.1405 s，STP 159.81 J、JFP 160.70 J，重建總和320.51 J。官方POI總轉移平均338.94 J；重建總和與官方值在球層級r=0.971、投手層級r=0.974，但平均少18.43 J，因此以下以官方總轉移為主要結果，full-signal總平均功率只作其時序組成預測量，不把重建值冒充官方值。

| 官方總轉移投手層級模型 | R² | adjusted R² | 重複10-fold CV R²（平均） |
|---|---:|---:|---:|
| 體重 | 0.469 | 0.464 | 0.313 |
| 體重＋FP–BR時間 | 0.491 | 0.481 | 0.290 |
| 體重＋重建平均總功率 | 0.734 | 0.729 | 0.652 |
| 體重＋FP–BR時間＋重建平均總功率 | 0.936 | 0.934 | 0.899 |

完整模型的標準化係數為重建平均總功率β=0.891、FP–BR時間β=0.515、體重β=0.158，三者p值皆`<0.001`。411球 mixed-effects 敏感度模型將時間與平均功率各拆成投手間平均及投手內偏差後，兩者的投手間與投手內係數也都為正且p值皆`<0.001`。

整段總能量與軀幹限制期STP的結論不同：整段官方總轉移主要由平均功率區分，持續時間仍提供重要的額外預測；軀幹限制期STP則以維持該限制狀態多久為主要組成。兩者都是能量積分的組成分解，不是平均功率或時間造成能量的因果證據。對完整full-signal重建值，`能量 = pooled mean power × 平均FP–BR時間`是精確恆等式；官方POI值因與重建積分不完全相同，三變項模型不會得到R²=1。
