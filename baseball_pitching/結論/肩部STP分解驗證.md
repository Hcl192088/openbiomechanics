...
於 `fp_poi_time` 至BR，先不處理左投符號時，411球、21,205幀的重建STP與官方上臂端STP為r=0.555、MAE=498.29 W；右投328球為r=0.832、MAE=283.33 W，左投83球為r=-0.067、MAE=1,355.61 W。依CMO明載的左側X/Z反轉，只檢驗相對肩速度與軀幹速度各自使用原值或反轉X/Z的四種位置；最佳為「左投相對肩速度反轉X/Z、軀幹速度維持原值」，全體改善至r=0.693、MAE=423.63 W，左投改善至r=0.467、MAE=983.18 W。右投結果支持向量加法方向正確，但兩側誤差仍不足以作三軸正式結論；左投也無法只靠CMO表面的X/Z規則完整對齊，表示未公開的私人匯出流程仍有轉換資訊缺口。不得以全體結果拆解上臂端STP。
...
單線圖的Y軸是從FP起一路累加的正向STP能量，因此只會上升；線段顏色表示該正規化時間點100位投手中哪一側較常限制，綠色為上臂、橘色為軀幹。軀幹旋轉速度 `abs(torso_velo_z)` 的個人峰值中位在FP–BR 38.8%（IQR 34.1–45.3%），此時曲線仍為上臂限制主導；群體主要限制端約在70%附近轉為軀幹，最後持續交接中位為72.3%，MER中位77.1%（IQR 74.5–79.0%）。到BR時累積正向STP為159.99 J，其中上臂限制87.63 J、軀幹限制72.36 J；因此前段與整體略偏上臂限制，接近MER及後段則偏軀幹限制。顏色是群體多數狀態，不代表每位投手都在同一瞬間切換。
